...
GWPCR$lambda.def <- list(list(to=0.025,by=0.0025),
                         ...
                         list(to=15,   by=0.5),
                         list(to=50,   by=5))
GWPCR$efficiency.def <- list(list(to=0.000001,by=0.000001),
                             list(to=0.00001,by=0.000009),
                             list(to=0.0001, by=0.00009),
                             list(to=0.001,  by=0.0009),
                             list(to=0.01,   by=0.009),
...
# Generate efficiency grid from grid definition. The first entry uses
# a gamma approximation, the rest are generated by simulation
GWPCR$efficiency <- tail(make.grid(GWPCR$efficiency.def), -1)
GWPCR$efficiency.usegamma <- (GWPCR$efficiency < 0.01)
...
for(e in 1:length(GWPCR$efficiency)) {
  ...
  if (!GWPCR$efficiency.usegamma[e]) {
    # Generate density (d) at points GWPCR$lambda by simulation followed by
    # density estimation.
    ...
    message('Efficiency=', efficiency, ': Using gamma approximation')
    # Generate using gamma approximation. The gamma parameters are chosen
    # to produce a distribution with mean 1 and
    #               1 - E
    #   Var( L ) = ------- ,
    #               1 + E
    # which matches the variance of the true distribution (see gwpcr.sd).
    d <- dgamma(GWPCR$lambda,
                shape = (1+efficiency)/(1-efficiency),
                rate = (1+efficiency)/(1-efficiency))
  }
